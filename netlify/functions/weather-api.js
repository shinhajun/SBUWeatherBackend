// netlify/functions/weather-api.js
...
// 메모리에 예보 및 가중치 저장 (예시)
// ※ Netlify Functions는 인메모리 상태가 함수 호출 간 유지되지 않으므로,
//    이 코드는 단발성 요청 시 초기화됩니다.
...
/*
  Netlify Functions는 상태 유지나 장시간 실행(setInterval, setTimeout)이 보장되지 않습니다.
  아래 스케줄링 코드는 Express 앱 내부에 포함되어 있으나,
  매 HTTP 요청마다 새로운 인스턴스에서 실행되므로
  실제로는 지속적으로 동작하지 않을 수 있습니다.
  (지속적인 업데이트가 필요하다면 외부 스케줄러를 사용해야 합니다.)
*/
startUpdateSchedule();
